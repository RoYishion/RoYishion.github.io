const scoreService = require('../service/rolesService');

module.exports = {
  getAllRoles: async (ctx, next) => {
    try {
      let result = await scoreService.getAllRoles()
      if (result) {
        ctx.body = {
          success: true,
  ...
  addRole: async (ctx, next) => {
    let addRoles = await scoreService.addRole(ctx.request.body);
    if (addRoles) {
      ctx.body = {
        success: true,
  ...
  deleteRole: async (ctx, next) => {
    let deleteRole = await scoreService.deleteRole(ctx.params.id);
    if (deleteRole) {
      ctx.body = {
        success: true,
  ...
  updateRole: async (ctx, next) => {
    let updateRole = await scoreService.updateRole(ctx.request.body);
    if (updateRole) {
      ctx.body = {
        success: true,

...
module.exports = {
  ...
  registerUser: async (userInfo) => {
    if (userInfo.phone) {
      ...
      if (!user) {
        ...
        let result = await userDao.createUser(userInfo);
        if (result) {
          return { success: 1, info: '注册用户成功' }
        } else {
          return { success: 0, info: '注册用户失败' }
        }
      } else {
        return { success: 0, info: '该手机号码已存在' }
      }
  ...
  updateUser: async (userInfo) => {
    if (userInfo[0].phone) {
      if (userInfo[0].phone == userInfo[1].oldPhone) {
        let result = await userDao.updateUser(userInfo[0])
        if (result) {
        ...
        let user = await userDao.getUserByPhone(userInfo[0].phone);
        if (!user) {
          let result = await userDao.updateUser(userInfo[0])
          if (result) {
            return { updateuser: userInfo, success: 1, info: '修改用户成功' };
